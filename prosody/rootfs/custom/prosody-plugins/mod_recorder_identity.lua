...
local room_jid_match_rewrite_without_resource = module:require "util".room_jid_match_rewrite_without_resource;
...
module:hook("pre-iq/full", function(event)
    ...
    if stanza.name == "iq" then
        ...
        if jibri and jibri.attr.action == 'start' and jibri.attr.recording_mode == 'file' then
            ...
            local room1 = room_jid_match_rewrite_without_resource(stanza.attr.to);
            local room2 = get_room_from_jid(room_jid_match_rewrite_without_resource(stanza.attr.to));
            ...
            if recorder_identity then
                ...
                local room = get_room_from_jid(room_jid_match_rewrite_without_resource(stanza.attr.to));
